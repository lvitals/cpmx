...
#define OPCODE_RET  0xC9  // return from subroutine

// Offsets for register pairs: (b,c), (d,e), (h,l), (sp)
const size_t reg_offsets[8] = { 1, 0, 3, 2, 5, 4, 8, 9 }; //bcdehlfa

// Struct to represent the state of the CPU registers
struct registers
{
    ...
    // Flags for the CPU. Must remain in this order for getFlag() to work
    ...
    // Function to materialize the flag byte from the individual flags
    ...
    // Return the Processor Status Word (PSW)
    uint16_t PSW()
    {
        materializeFlags();
        return * ( (uint16_t *) & f );
    ...
    // Increment the refresh register (Z80-specific)
    void z80_increment_r() { /* reg.r++; */ } // 4.6% of runtime when the increment is enabled

    // Update the individual flags from the flag byte
    ...
    // Set the Processor Status Word (PSW)
    ...
    // Return the address of a register pair (B, D, H, SP)
    ...
    // Return the address of a register pair from an opcode
    uint16_t * rpAddressFromOp( uint8_t op )
    {
        return (uint16_t *) ( ( (uint8_t *) this ) + ( ( op >> 3 ) & 6 ) );
    } //rpAddressFromOp

    // Return the address of a register pair from a low opcode
    ...
    // Return the address of a register based on its index
    ...
    // Get the flag value based on its index
    ...
    // Render the flag values as a string

...
//     cl /nologo /openmp cpmx.cxx x80.cxx /Oti2 /Ob2 /Qpar /Fa /EHac /Zi /DDEBUG /D_AMD64_ /link user32.lib ntdll.lib /OPT:REF
//     cl /nologo /openmp cpmx.cxx x80.cxx /Oti2 /Ob2 /Qpar /Fa /EHac /Zi /DNDEBUG /D_AMD64_ /link user32.lib ntdll.lib /OPT:REF
// To build on Linux debug and release:
//     g++ -ggdb -Ofast -fopenmp -fno-builtin -D DEBUG -I . cpmx.cxx x80.cxx -lssl -lcrypto -o cpmx
//     g++ -ggdb -Ofast -fopenmp -fno-builtin -D NDEBUG -I . cpmx.cxx x80.cxx -lssl -lcrypto -o cpmx
// To build on Windows with mingw64 & g++: (performance is >10% faster than the Microsoft compiler)
//     g++ -Ofast -ggdb -fopenmp -D _MSC_VER cpmx.cxx x80.cxx -I ../djl -D DEBUG -o cpmx.exe -static -lwininet
//     g++ -Ofast -ggdb -fopenmp -D _MSC_VER cpmx.cxx x80.cxx -I ../djl -D NDEBUG -o cpmx.exe -static -lwininet
...
//    wcl -q -zp=1 -ml -obmr -oh -ei -oi -s -0 -xs -j -oe=128 -ol+ -ot cpmx.cxx x80.cxx -bcl=DOS -k8192 /I. /DWATCOM /DNDEBUG
...
#include "x80.hxx"
#include "cpmx.hxx" // get bdos extensions
...
void dump_memory( const char * pname = "cpmx.dmp" )
{
...
{
    //dump_memory( "cpmx_hard_exit.dmp" );
...
void usage( char const * perr = 0 )
{
    ...
    printf( "CPMX: emulates a CP/M 2.2 i8080/Z80 runtime environment\n" );
    printf( "usage: cpmx [-c] [-p] [-s:X] [-t] <cp/m 2.2 .com file> [filearg1] [filearg2]\n" );
    ...
    printf( "         -t     enable debug tracing to cpmx.log\n" );
    ...
    printf( "       cpmx asm.com test\n" );
    printf( "       cpmx load.com test\n" );
    printf( "       cpmx test.com\n" );
    printf( "  e.g. to run Star Trek in mbasic in 80x24 mode using i8080 emulation:\n" );
    printf( "       cpmx -8 -C mbasic startrek.bas\n" );
    printf( "  %s\n", build_string() );
    exit( -1 );
} //usage
...
int main( int argc, char * argv[] )
{
    try
    {
        ...
        tracer.Enable( trace, L"cpmx.log", true );
        ...
        * pCommandTailLen = (char) strlen( pCommandTail );
        ...
        if ( ends_with( acCOM, "pasopt.com" ) )
            memory[ BDOS_ENTRY - 0x30e ] = 0xff; // address 0xfbee for CPMX's BDOS address. the value is arbitrary non-zero.
        ...
        memory[0] = OPCODE_JMP;    // jump to warm boot, which likely just exits cpmx unless overridden by an app.
        ...
        if ( showPerformance )
        {
...
#endif
            printf( "elapsed milliseconds: %16s\n", TraceLogger::RenderNumberWithCommas( elapsedMS, ac ) );
            printf( "%s cycles:      %20s\n", reg.fZ80Mode ? "Z80 " : "8080", TraceLogger::RenderNumberWithCommas( total_cycles, ac ) );
            ...
            if ( 0 == clockrate )
            {
                ...
                if ( reg.fZ80Mode )
                    printf( "approx ms at 4Mhz: %19s", TraceLogger::RenderNumberWithCommas( total_ms, ac ) );
                else
                    printf( "approx ms at 2Mhz: %19s", TraceLogger::RenderNumberWithCommas( total_ms, ac ) );
                ...
                printf( "      %20s Hz\n", TraceLogger::RenderNumberWithCommas( clockrate, ac ) );
        }
    }
    catch ( bad_alloc & e )
    {
